...
const ProfilePage = () => {
    ...
    return (
        <div>
            <ProfileComp1/>
            <div className="flex flex-col my-10 gap-4 justify-center items-center">
                <div>
                    <span className="font-semibold">User Fullname: </span>
                    {user ? user.fullName : "<h1>User fullname not found</h1>"}
                </div>
                <div>
                    <span className="font-semibold">User email: </span>
                    {user ? user.email : "<h1>User email not found</h1>"}
                </div>
                <button onClick={handleLogout} className="border-2 border-black px-10 py-2 rounded-full bg-amber-200 hover:text-2xl">
                    {isLoading ? (
                        <div>
                            <svg
